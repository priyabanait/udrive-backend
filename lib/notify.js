  ...
  try {
    ...
    try {
      ...
      if (normalizedRecipientType && normalizedRecipientId) {
        ...
        console.log(
          `Notification also emitted to room: ${normalizedRecipientType}:${normalizedRecipientId}`
        );
      }
        ...
        if (normalizedRecipientType && normalizedRecipientId) {
          ...
          try {
            if (normalizedRecipientType === 'driver') {
              targetExists = !!(await Driver.exists({ _id: String(normalizedRecipientId) }));
            } else if (normalizedRecipientType === 'investor') {
              targetExists = !!(await Investor.exists({ _id: String(normalizedRecipientId) }));
            }
          } catch (ve) {
          ...
          if (!targetExists) {
            ...
            console.log(
              `[NOTIFY] Found ${tokens.length} device token(s) for ${normalizedRecipientType}:${normalizedRecipientId}`
            );
          }
          ...
          tokens = await DeviceToken.find({ userType: normalizedRecipientType }).distinct("token");
              ...
              if (tokens && tokens.length) {
                ...
                if (result && result.invalidTokens && result.invalidTokens.length > 0) {
                  try {
                    ...
                    console.log(`🧹 Cleaned up ${result.invalidTokens.length} invalid tokens for driver ${driverId}`);
                  } catch (cleanupErr) {
