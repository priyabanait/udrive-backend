...
export async function createAndEmitNotification({
  ...
  try {
    ...
    console.log("[NOTIFY] Saved RecipientType:", note.recipientType);
    console.log(
    ...
    try {
      ...
      if (recipientType && normalizedRecipientId) {
        io.to(`${recipientType}:${normalizedRecipientId}`).emit(
          "dashboard:notification",
          note
        );
        console.log(
          `Notification also emitted to room: ${recipientType}:${normalizedRecipientId}`
        );
      }
      ...
      try {
        let tokens = [];
        if (recipientType && normalizedRecipientId) {
          console.log("[NOTIFY] Searching for tokens with criteria:", {
            userType: recipientType,
            userId: String(normalizedRecipientId),
          });
          ...
          console.log(
            `[NOTIFY] Found ${tokens.length} device token(s) for ${recipientType}:${normalizedRecipientId}`
          );
        } else {
          console.log(
            "[NOTIFY] No recipient specified - searching for all tokens"
          );
          tokens = await DeviceToken.find({}).distinct("token");
          console.log(`[NOTIFY] Found ${tokens.length} total device token(s)`);
        }
...
export async function listNotifications({
  ...
  if (driverId) {
    ...
    console.log(
      `[NOTIFY] Fetching notifications for type: ${recipientType}, id: ${normalizedRecipientId}`
    );
    // For specific recipient type/id: show their notifications + broadcast to that type only (NO global notifications)
    query = {
      $or: [
        { recipientType, recipientId: normalizedRecipientId }, // Specific to this user
        { recipientType, recipientId: null }, // Broadcast to all users of this type
      ],
    };
  } else {
...
export async function markAllAsRead({
  ...
  if (recipientType && recipientId) {
    ...
    query = {
      recipientType, // All notifications for this type (both specific and broadcast, but not global)
    };
  } else {
        ...
        try {
          ...
          try {
            ...
            try {
              ...
              console.log(
                `📱 Found ${tokens.length} device token(s) for investor ${investorId}`
              );
              ...
              if (tokens && tokens.length) {
                ...
                if (result.invalidTokens && result.invalidTokens.length > 0) {
                  try {
                    ...
                    console.log(
                      `🧹 Cleaned up ${result.invalidTokens.length} invalid tokens for investor ${investorId}`
                    );
                  } catch (cleanupErr) {
                    console.warn(
