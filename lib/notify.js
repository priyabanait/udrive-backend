...
export async function listNotifications({ page = 1, limit = 20, driverId = null, investorId = null, recipientType = null, recipientId = null } = {}) {
  ...
  if (driverId) {
    // For drivers: show their specific notifications + broadcast to all drivers + global notifications
    query = {
      $or: [
        { recipientType: 'driver', recipientId: driverId }, // Specific to this driver
        { recipientType: 'driver', recipientId: null }, // Broadcast to all drivers
        { recipientType: null, recipientId: null } // Global notifications
      ]
    };
  } else if (investorId) {
    // For investors: show their specific notifications + broadcast to all investors + global notifications
    query = {
      $or: [
        { recipientType: 'investor', recipientId: investorId }, // Specific to this investor
        { recipientType: 'investor', recipientId: null }, // Broadcast to all investors
        { recipientType: null, recipientId: null } // Global notifications
      ]
    };
  } else if (recipientType && recipientId) {
    // For specific recipient type/id: show their notifications + broadcast to that type + global notifications
    query = {
      $or: [
        { recipientType, recipientId }, // Specific to this user
...
export async function markAllAsRead({ recipientType = null, recipientId = null } = {}) {
  // Build query similar to listNotifications: specific recipient + broadcast to type + global notifications
  let query = {};
  if (recipientType && recipientId) {
    query = {
      $or: [
    ...
    query = {
      $or: [
        { recipientType }, // All notifications for this type (both specific and broadcast)
        { recipientType: null } // Global notifications
      ]
    };
  } else {
