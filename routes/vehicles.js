...
router.get("/investor/:investorId", async (req, res) => {
  try {
    ...
    const vehicles = await Vehicle.find({ investorId })
...
router.get("/", async (req, res) => {
  try {
    ...
    const enhancedList = list.map((vehicle) => {
      ...
      if (
        Array.isArray(normalized.rentPeriods) &&
        normalized.rentPeriods.length > 0 &&
        status === "active"
      ) {
  ...
  try {
    ...
    if (vehicleData.investorId) {
      ...
      if (mongoose.default.Types.ObjectId.isValid(vehicleData.investorId)) {
        vehicleData.investorId = new mongoose.default.Types.ObjectId(
          vehicleData.investorId
        );
      } else {
    ...
    try {
      const CarInvestmentEntry = (
        await import("../models/carInvestmentEntry.js")
      ).default;
      const entries = await CarInvestmentEntry.find();
      const entryNames = entries.map((e) =>
        (e.carname || "").trim().toLowerCase()
      );
      let cat = (vehicleData.category || "").trim().toLowerCase();
      let carCat = (vehicleData.carCategory || "").trim().toLowerCase();
      let match =
        entryNames.find((name) => name === cat) ||
        entryNames.find((name) => name === carCat);
      if (!match) {
        // Try to auto-correct using closest match (startsWith)
        let suggestion =
          entryNames.find((name) => cat && name.startsWith(cat.slice(0, 3))) ||
          entryNames.find(
            (name) => carCat && name.startsWith(carCat.slice(0, 3))
          );
        if (suggestion) {
          if (cat && !entryNames.includes(cat))
            vehicleData.category = suggestion;
          if (carCat && !entryNames.includes(carCat))
            vehicleData.carCategory = suggestion;
        }
      }
    } catch (err) {
    ...
    try {
      ...
      const category = (vehicleData.category || vehicleData.carCategory || "")
        .trim()
        .toLowerCase();
      ...
      if (matchedInvestment) {
        vehicleData.monthlyProfitMin = parseFloat(
          matchedInvestment.finalMonthlyPayout ||
            matchedInvestment.MonthlyPayout ||
            0
        );
      }
    } catch (err) {
    ...
    for (const field of photoFields) {
      if (
        vehicleData[field] &&
        typeof vehicleData[field] === "string" &&
        vehicleData[field].startsWith("data:")
      ) {
        try {
          const result = await uploadToCloudinary(
            vehicleData[field],
    ...
    try {
      ...
      await createAndEmitNotification({
        ...
        title: `New vehicle added - ${
          savedVehicle.registrationNumber || savedVehicle.vehicleId
        }`,
        message: `${
          savedVehicle.registrationNumber || savedVehicle.vehicleId
        } added to fleet.`,
    ...
    res
      .status(500)
      .json({ message: err?.message || "Failed to create vehicle" });
  }
});
  ...
  try {
    ...
    if (updates.investorId) {
      ...
      if (mongoose.default.Types.ObjectId.isValid(updates.investorId)) {
        updates.investorId = new mongoose.default.Types.ObjectId(
          updates.investorId
        );
      } else {
    ...
    try {
      ...
      if (!match) {
        ...
        if (suggestion) {
          ...
          if (carCat && !entryNames.includes(carCat))
            updates.carCategory = suggestion;
        }
      }
    } catch (err) {
      console.error("Error normalizing category:", err);
    ...
    try {
      ...
      if (matchedInvestment) {
        updates.monthlyProfitMin = parseFloat(
          matchedInvestment.finalMonthlyPayout ||
            matchedInvestment.MonthlyPayout ||
            0
        );
    ...
    const isNewDriverAssignment =
      !existing.assignedDriver && updates.assignedDriver;
    ...
    if (isNewDriverAssignment) {
      try {
        const DriverPlanSelection = (
          await import("../models/driverPlanSelection.js")
        ).default;
        const Driver = (await import("../models/driver.js")).default;
        const DriverSignup = (await import("../models/driverSignup.js"))
          .default;
        ...
        if (updates.assignedDriver) {
          ...
          try {
            const mongoose = await import("mongoose");
            if (
              mongoose.default.Types.ObjectId.isValid(updates.assignedDriver)
            ) {
          ...
          if (!driverSignup) {
            try {
              ...
              if (
                ...
                driverSignup = await DriverSignup.findById(
                  updates.assignedDriver
                ).lean();
              }
            } catch (err) {
              console.warn(
                "Failed to find driverSignup by ObjectId:",
                err.message
              );
            }
          }
          ...
          if (driverSignup) {
            ...
            assignedDriverName =
              driverSignup.username || driverSignup.mobile || "Driver";
          }
          ...
          if (driverSignup) {
            ...
            if (driverSignup.username)
              driverUsernames.push(driverSignup.username);
          ...
          if (!assignedDriverName && driver) {
            assignedDriverName =
              driver.name || driver.username || driver.mobile || "Driver";
          }
          ...
          assignedVehicleInfo = {
            vehicleId: vehicleId,
            registrationNumber:
              existing.registrationNumber || updates.registrationNumber || "",
          ...
          try {
            ...
            if (
              ...
              driverSignupIds.push(
                new mongoose.default.Types.ObjectId(updates.assignedDriver)
              );
            }
          } catch (err) {
            console.warn(
              "Failed to validate assignedDriver as ObjectId (collecting signup ids)",
              err.message
            );
          }
        }
        ...
        if (driverSignupIds && driverSignupIds.length > 0) {
          console.log(
            "(assignment) Collected driverSignupIds:",
            driverSignupIds.map((x) => String(x))
          );
        }
        // Log the driver mobile/username candidates for debugging
        console.log(
          "(assignment) driverMobiles:",
          driverMobiles,
          "driverUsernames:",
          driverUsernames
        );
        ...
        try {
          ...
          const matchedBefore = await DriverPlanSelection.find(
            updateQuery
          ).lean();
          console.log(
            `(assignment) Found ${matchedBefore.length} matching selections before update. Examples:`,
            matchedBefore.slice(0, 5).map((m) => ({
              _id: m._id,
              driverMobile: m.driverMobile,
              driverUsername: m.driverUsername,
              vehicleId: m.vehicleId,
              rentStartDate: m.rentStartDate,
            }))
          );
          if (matchedBefore.length === 0 && updates.assignedDriver) {
            ...
            try {
              const fallback = await DriverPlanSelection.find({
                ...
                .lean();
              console.log(
                `(assignment) Fallback found ${fallback.length} selections (regex on assignedDriver). Examples:`,
                fallback.map((m) => ({
                  _id: m._id,
                  driverMobile: m.driverMobile,
                  driverUsername: m.driverUsername,
              ...
              console.error(
                "Error running fallback debug query (assignment):",
                err.message
              );
            }
          }
          ...
          console.error(
            "Error listing matching selections before update (assignment):",
            err.message
          );
        }

        const result = await DriverPlanSelection.updateMany(updateQuery, {
          $set: {
            vehicleId: vehicleId,
        ...
        console.log(
          `Updated ${result.modifiedCount} driver plan selections with vehicleId and rentStartDate for newly assigned driver on vehicle ${vehicleId}`
        );
        ...
        try {
          if (existing.status !== "active") {
            ...
            console.log(
              `Auto-activated vehicle ${vehicleId} and started rent due to driver assignment.`
            );
          }
        } catch (err) {
          console.warn(
            "Failed to auto-activate vehicle after assignment:",
            err.message
          );
        }
      } catch (err) {
        console.error(
          "Error updating driver plan selections for new driver assignment:",
          err
        );
      }
    }
      ...
      try {
        ...
        if (assignedDriver) {
          ...
          try {
            ...
            console.warn(
              "Assigned driver id lookup failed (activation):",
              err.message
            );
          }
          ...
          const driverSignup = await DriverSignup.findOne({
            $or: [{ username: assignedDriver }, { mobile: assignedDriver }],
          }).lean();
          ...
          try {
            ...
            if (mongoose.default.Types.ObjectId.isValid(assignedDriver)) {
              driverSignupIds.push(
                new mongoose.default.Types.ObjectId(assignedDriver)
              );
            }
          } catch (err) {
            console.warn(
              "Failed to validate assignedDriver as ObjectId (activation signup ids)",
              err.message
            );
          }
        }
        ...
        if (driverUsernames.length > 0) {
          updateQuery.$or.push({
            driverUsername: {
              $in: driverUsernames.map(
                (u) => new RegExp(`^${escapeRegex(u)}$`, "i")
              ),
            },
          });
        }
        ...
        if (driverSignupIds && driverSignupIds.length > 0) {
          ...
          updateQuery.$or.push({
            driverSignupId: { $in: driverSignupIds.map((x) => x.toString()) },
          });
          console.log(
            "(activation) Collected driverSignupIds:",
            driverSignupIds.map((x) => String(x))
          );
        }
        ...
        try {
          ...
          console.log(
            `(activation) Found ${matchedBefore.length} matching selections before update. Examples:`,
            matchedBefore.slice(0, 5).map((m) => ({
              _id: m._id,
              driverMobile: m.driverMobile,
              driverUsername: m.driverUsername,
          ...
          if (matchedBefore.length === 0 && assignedDriver) {
            ...
            try {
              const fallback = await DriverPlanSelection.find({
                ...
                .lean();
              console.log(
                `(activation) Fallback found ${fallback.length} selections (regex on assignedDriver). Examples:`,
                fallback.map((m) => ({
                  _id: m._id,
                  driverMobile: m.driverMobile,
                  driverUsername: m.driverUsername,
              ...
              console.error(
                "Error running fallback debug query (activation):",
                err.message
              );
            }
          }
          ...
          console.error(
            "Error listing matching selections before update (activation):",
            err.message
          );
        }
        ...
        console.log(
          `Updated ${result.modifiedCount} driver plan selections to active for vehicle ${vehicleId}`
        );
      } catch (err) {
      ...
      try {
        ...
        if (assignedDriver) {
          ...
          try {
            ...
            console.warn(
              "Assigned driver id lookup failed (inactivation):",
              err.message
            );
          }
        ...
        if (driverSignupIds && driverSignupIds.length > 0) {
          ...
          console.log(
            "(inactivation) Collected driverSignupIds:",
            driverSignupIds.map((x) => String(x))
          );
        }
        ...
        selectionsUpdated += result.modifiedCount || 0;
        console.log(
          `Updated ${result.modifiedCount} driver plan selections to inactive for vehicle ${vehicleId}`
        );
      } catch (err) {
        console.error("Error updating driver plan selections:", err);
      }
    ...
    if (isNewDriverAssignment && assignedDriverSignupId) {
      try {
        ...
        const vehicleDetails = `${
          assignedVehicleInfo.carName ? assignedVehicleInfo.carName + " " : ""
        }${assignedVehicleInfo.model || ""} (${
          assignedVehicleInfo.registrationNumber || "N/A"
        })`.trim();
        ...
        console.log(
          `✅ Notification sent to driver ${assignedDriverSignupId} for vehicle assignment ${assignedVehicleInfo.vehicleId}`
        );
      } catch (notifyErr) {
        console.warn(
    ...
    res
      .status(500)
      .json({ message: err?.message || "Failed to update vehicle" });
  }
});
...
router.get("/:id/monthly-profit", async (req, res) => {
  try {
    ...
    const CarInvestmentEntry = (await import("../models/carInvestmentEntry.js"))
      .default;
    const category = (
      vehicle.category ||
      vehicle.carCategory ||
    ...
    const matchedInvestment = await CarInvestmentEntry.findOne({
    ...
    if (vehicle.monthlyProfitMin && vehicle.monthlyProfitMin > 0) {
      ...
      monthlyProfit =
        (matchedInvestment.minAmount * (matchedInvestment.expectedROI / 100)) /
        12;
    }
